...
public class PulsarWithTlsAuthRecordsStorageTest extends PulsarAbstractStorageTest implements RecordStorageTests {
    ...
    static {
        ...
        System.getProperties().putAll(Map.of(
                ...
                "pulsar.authPluginClassName", "org.apache.pulsar.client.impl.auth.AuthenticationTls",
                "pulsar.authPluginParams.tlsCertFile", pulsar.getUserCert().toAbsolutePath().toString(),
                "pulsar.authPluginParams.tlsKeyFile", pulsar.getUserKey().toAbsolutePath().toString()
        ));
